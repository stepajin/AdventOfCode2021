...
func paths(cave: Cave, visited: [Cave: Bool]) -> Int {
    adjacentCaves[cave]!.map { cave -> Int in
        ...
        case .start:
            return 0
        case .big:
            return paths(cave: cave, visited: visited)
        case .small where visited[cave] == nil:
            var _visited = visited
            _visited[cave] = true
            return paths(cave: cave, visited: _visited)
        case .small:
            return 0
        }
    }.reduce(0, +)
...
let count = paths(cave: .start, visited: [:])
print(count)
