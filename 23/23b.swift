...
let rooms: [[Index]] = [3, 5, 7, 9].map { x in [2, 3, 4, 5].map { (x: x, y: $0) } }
...
func canMove(_ grid: Grid, _ pod: Pod, toRoom room: Index) -> Bool {
    isEmpty(grid, room.x, room.y) && (room.y == 5 || stride(from: 5, to: room.y, by: -1).allSatisfy { grid[$0][room.x] == pod })
}
...
initialGrid.insert(Array("  #D#C#B#A#  "), at: 3)
initialGrid.insert(Array("  #D#B#A#C#  "), at: 4)
